import os
import re
import string
import pandas as pd
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity
import json

# Load and preprocess the dataset
documents = []
base_dir = 'dataset/'

# Function to preprocess text
def preprocess_text(text):
    text = text.lower()
    text = re.sub(r'\d+', '', text)
    text = text.translate(str.maketrans('', '', string.punctuation))
    text = text.strip()
    return text

# Traverse through the dataset directory structure
known_sections = {}
for section_folder in os.listdir(base_dir):
    section_path = os.path.join(base_dir, section_folder)
    if os.path.isdir(section_path):
        known_sections[section_folder] = []
        for clause_file in os.listdir(section_path):
            clause_path = os.path.join(section_path, clause_file)
            if os.path.isfile(clause_path):
                with open(clause_path, 'r', encoding='utf-8') as file:
                    content = file.read().strip()
                    section_name = section_folder
                    clause_name = os.path.splitext(clause_file)[0]
                    
                    # Split content by the delimiter ###xxx###
                    sub_clauses = content.split("###xxx###")
                    for sub_clause in sub_clauses:
                        # Find all matches of the pattern ###Sub-section name###Sub-clause text
                        matches = re.findall(r"###(.+?)###(.+)", sub_clause, re.DOTALL)
                        if matches:
                            for match in matches:
                                sub_section_name = match[0].strip()
                                clause_text = match[1].strip()
                                if clause_text:
                                    documents.append({
                                        "text": preprocess_text(clause_text),
                                        "section": section_name,
                                        "clause": clause_name,
                                        "sub_section": sub_section_name
                                    })
                        else:
                            # No sub-section name, just use the entire sub_clause text
                            clause_text = sub_clause.strip()
                            if clause_text:
                                documents.append({
                                    "text": preprocess_text(clause_text),
                                    "section": section_name,
                                    "clause": clause_name,
                                    "sub_section": None
                                })
                    known_sections[section_folder].append(clause_name)

# Convert to DataFrame for easier handling
df = pd.DataFrame(documents)

# Vectorize the dataset using TF-IDF
vectorizer = TfidfVectorizer()
X = vectorizer.fit_transform(df['text'])

def predict_section(input_text):
    # Preprocess the input text
    input_text_preprocessed = preprocess_text(input_text)

    # Vectorize the input text
    input_text_vectorized = vectorizer.transform([input_text_preprocessed])

    # Calculate similarity for clauses within the same section
    similarities = {}
    for section_name in df['section'].unique():
        section_indices = df[df['section'] == section_name].index
        section_X = X[section_indices]
        section_similarities = cosine_similarity(input_text_vectorized, section_X)
        max_similarity = section_similarities.max()
        max_similarity_index = section_indices[section_similarities.argmax()]
        similarities[section_name] = (max_similarity, max_similarity_index)

    # Find the most similar clause across all sections
    most_similar_section = max(similarities, key=lambda x: similarities[x][0])
    most_similar_clause_index = similarities[most_similar_section][1]
    most_similar_clause = df.loc[most_similar_clause_index]

    # Prepare the result as a dictionary
    result = {
        "section": most_similar_clause['section'],
        "clause": most_similar_clause['clause'],
        "sub_section": most_similar_clause['sub_section'] if most_similar_clause['sub_section'] else None
    }
    return result

# Function to split the uploaded document into sections based on known sections and sub-sections
def split_document_into_sections(document_text):
    sections = []
    
    # Generate regex pattern to match known sections and sub-sections
    section_patterns = [re.escape(sec) for sec in known_sections.keys()]
    pattern = re.compile(r'(' + '|'.join(section_patterns) + r')', re.IGNORECASE)
    
    # Find all sections in the document
    split_points = [(m.start(), m.group()) for m in pattern.finditer(document_text)]
    split_points.append((len(document_text), ''))  # Add end of document as a split point
    
    for i in range(len(split_points) - 1):
        start, section_name = split_points[i]
        end, _ = split_points[i + 1]
        section_text = document_text[start:end].strip()
        
        if section_text:
            section_prediction = predict_section(section_text)
            sections.append({
                "original_text": section_text,
                "predicted_section": section_prediction
            })
    
    return sections

# Example usage:
uploaded_document = """
General Information

This is the general information section.

Detailed Clauses

This section contains detailed clauses.
"""

# Split the document into sections and predict their names
sections = split_document_into_sections(uploaded_document)

# Convert the results to JSON
sections_json = json.dumps(sections, indent=4)
print(sections_json)
